...
class ReporteController {
  ...
  static async obtenerEstadisticasSacramentos(req, res, next) {
    try {
      // Datos simulados de sacramentos (en un sistema real vendrían de la base de datos)
      const estadisticasSacramentos = {
        bautizo: 234,
        confirmacion: 67,
        matrimonios: 150,
        fecha_generacion: new Date().toISOString()
      ...
      // Calcular total
      const total = estadisticasSacramentos.bautizo + estadisticasSacramentos.confirmacion + estadisticasSacramentos.matrimonios;
      
      // Calcular porcentajes
      const porcentajeBautizo = total > 0 ? ((estadisticasSacramentos.bautizo / total) * 100).toFixed(2) : 0;
      const porcentajeConfirmacion = total > 0 ? ((estadisticasSacramentos.confirmacion / total) * 100).toFixed(2) : 0;
      const porcentajeMatrimonios = total > 0 ? ((estadisticasSacramentos.matrimonios / total) * 100).toFixed(2) : 0;
      
      res.json({
        ok: true,
        mensaje: 'Estadísticas de sacramentos obtenidas correctamente',
        datos: {
          total_registros: total,
          sacramentos: {
            bautizo: {
              cantidad: estadisticasSacramentos.bautizo,
              porcentaje: porcentajeBautizo
            },
            confirmacion: {
              cantidad: estadisticasSacramentos.confirmacion,
              porcentaje: porcentajeConfirmacion
            },
            matrimonios: {
              cantidad: estadisticasSacramentos.matrimonios,
              porcentaje: porcentajeMatrimonios
            }
          },
          fecha_generacion: estadisticasSacramentos.fecha_generacion
        }
      });
